    ...
    for x in range(H):
        for y in range(W):
            if is_inside(x, y, bord_up, bord_down, bord_left, bord_right) and not(key_exist(info_on_pix, (x, y))):
                lost_map[x, y] = 1
    lost_map = lost_map.astype(np.uint8)
    # _, label_lost_map = cv2.connectedComponents(lost_map, connectivity=4)
    _, label_lost_map = cv2.connectedComponents(lost_map)
    mask = np.zeros((H, W))
...
def convert_to_texture_mesh(input_mesh, info_on_pix, config, ply_flag):
    vertex_id = 0
    node_str_point = []
    node_str_color = []
    node_str_uv = []
    ...
    for pix_xy, pix_list in info_on_pix.items():
        for pix_idx, pix_info in enumerate(pix_list):
            ...
            str_pt = [out_fmt(x, ply_flag) for x in reproject_3d_int_detail(pix_xy[0], pix_xy[1], pix_depth,
                                                                            k_00, k_02, k_11, k_12, w_offset, h_offset)]
            str_uv = [out_fmt(x, ply_flag) for x in pix_info['uv']]
            if input_mesh.has_node((pix_xy[0], pix_xy[1], pix_info['depth'])) is False:
                print("If this line pass?")
                return False
                continue
            ...
            if ply_flag is True:
                ...
                node_str_color.append(str_color)
                node_str_point.append(str_pt)
                node_str_uv.append(str_uv)
    str_faces = generate_face(input_mesh, info_on_pix, config)
    if ply_flag:
        return node_str_list, str_faces
    return node_str_point, node_str_color, node_str_uv, str_faces
...
def write_ply(image,
    ...
    # m = convert_to_texture_trimesh(np.asarray(node_str_point), str_faces, node_str_uv, Image.fromarray(image))
    # m.show()
    # write_glb('texture_sakura.glb', m)
